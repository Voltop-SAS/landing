...
| 3     | **Másteres sin subtítulos quemados + 3 `.vtt`** — ahora son **DOS** piezas | Único incumplimiento WCAG AA vivo. A la película se le suma `ceoVision` (2026-09-08): sus subtítulos van quemados y **en inglés**, así que en `/es` y `/pt` están en el idioma equivocado y no se pueden quitar                                                    |
