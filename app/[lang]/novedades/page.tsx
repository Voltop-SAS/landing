...
export default async function NovedadesPage({ params }: Props) {
  ...
  return (
    <TrackView event="novedades_vista" threshold={0}>
      {/* 1 · APERTURA + PULSO */}
      <Section space="tight" className="pt-32 md:pt-40">
      ...
      {posts.length === 0 ? (
        ...
        <>
          ...
          {/* 3 · EL REGISTRO */}
          {rest.length > 0 && (
            <Section space="tight" className="pb-24 md:pb-32">
              <Container>
                <PostLog posts={rest} lang={lang} />
              </Container>
            </Section>
          )}
        </>
